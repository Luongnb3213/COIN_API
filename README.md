...
```text
POST /transactions/gift-codes/grant                # gift_code_1 nếu có
POST /transactions/gift-codes/grant                # gift_code_2 nếu có
...
Nếu không có `gift_code_1` và `gift_code_2`, tool bỏ qua grant gift code và đi
thẳng sang bước lấy thông tin thẻ. `tocapi.coinplus.jp` dùng header app và
`XSRF-TOKEN`; webview dùng cookie `SESSION` riêng do server cấp qua
`Set-Cookie`.
